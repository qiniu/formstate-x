...
describe('DebouncedFieldState', () => {
  ...
  it('should onChange well', async () => {
    const initialValue = ''
    const state = createFieldState(initialValue).addValidator(
...
describe('DebouncedFieldState validation', () => {
  it('should work well when initialized', async () => {
    const state = createFieldState('').addValidator(val => !val && 'empty')
  ...
  it('should work well with onChange()', async () => {
    const state = createFieldState('xxx').addValidator(val => !val && 'empty')
    state.$.onChange('')
  ...
  it('should work well with onChange of same value', async () => {
    const state = createFieldState(1).addValidator(
      () => null
    )
  ...
  it('should work well with validate()', async () => {
    ...
    const state = createFieldState('').addValidator(validator)
  ...
  it('should work well with reset()', async () => {
    const initialValue = ''
    const state = createFieldState(initialValue).addValidator(val => !val && 'empty')
    state.validate()
    await delay()
  ...
  it('should work well with multiple validators', async () => {
    const state = createFieldState('').addValidator(
      val => !val && 'empty',
  ...
  it('should work well with race condition caused by validate()', async () => {
    ...
    const field = createFieldState(1).addValidator(validator)
    field.validate()
    await delay(50)
  ...
  it('should work well with NaN', async () => {
    const state = createFieldState(Number.NaN).addValidator(
      () => 'error'
    )
    state.validate()
    await delay()
  ...
  it('should validate with no delay', () => {
    ...
    const state = createFieldState('0').addValidator(validator)
    state.validate()
    expect(validator).toBeCalled()
  })
  ...
  it('should auto validate with delay', async () => {
    const validator = jest.fn(() => 'foo')
    const state = createFieldState(0, 500).addValidator(validator)
    state.onChange(1)
